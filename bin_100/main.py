...
def main():
    ...
    print("cohort numerosity:{0} -- max_seq_length:{1}".format(len(data), len(data[0][0])))
    model = net.ehrEncoding(vocab_size, len(data[0][0]), model_pars['embedding_dim'], model_pars['kernel_size'])
    ...
    lab, mrn, encoded, metrics_avg = train_and_evaluate(model, data_generator, loss_fn, optimizer, experiment_folder, metrics)
